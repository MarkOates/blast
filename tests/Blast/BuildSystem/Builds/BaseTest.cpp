...
class BuildStageTestClass : public Blast::BuildSystem::BuildStages::Base
{
public:
   int call_counter;
   BuildStageTestClass() : Blast::BuildSystem::BuildStages::Base("BuildStageTestClass"), call_counter(0) {}
   virtual bool execute() override { call_counter++; return true; }
};


class ExceptionThrowingBuildStageTestClass : public Blast::BuildSystem::BuildStages::Base
{
private:
...
public:
   ExceptionThrowingBuildStageTestClass(bool sleep_a_tiny_bit=false)
      : Blast::BuildSystem::BuildStages::Base("ExceptionThrowingBuildStageTestClass")
   {}
...
class FailingBuildStageTestClass : public Blast::BuildSystem::BuildStages::Base
{
private:
   bool sleep_a_tiny_bit;
...
public:
   FailingBuildStageTestClass(bool sleep_a_tiny_bit=false)
      : Blast::BuildSystem::BuildStages::Base("FailingBuildStageTestClass")
   {}
   virtual bool execute() override
   {
      if (sleep_a_tiny_bit) std::this_thread::sleep_for(std::chrono::milliseconds(100));
...
class SleepingBuildStageTestClass : public Blast::BuildSystem::BuildStages::Base
{
private:
   float length_in_seconds;
public:
   SleepingBuildStageTestClass(float length_in_seconds)
      : Blast::BuildSystem::BuildStages::Base("SleepingBuildStageTestClass")
...
class BuildsBaseTestClass : public Blast::BuildSystem::Builds::Base
{
public:
   BuildsBaseTestClass()
      : Blast::BuildSystem::Builds::Base("BuildsBaseTestClass")
   {}
};


TEST(Blast_BuildSystem_Builds_BaseTest, can_be_created_without_blowing_up)
{
   Blast::BuildSystem::Builds::Base base;
}


TEST(Blast_BuildSystem_Builds_BaseTest, has_the_expected_type)
{
   Blast::BuildSystem::Builds::Base base;
...
TEST(Blast_BuildSystem_Builds_BaseTest, derived_classes_will_have_the_expected_type)
{
...
TEST(Blast_BuildSystem_Builds_BaseTest, status__is_initialized_with_the_expected_value)
{
   BuildsBaseTestClass base_build;
   EXPECT_EQ(Blast::BuildSystem::Builds::Base::STATUS_WAITING_TO_START, base_build.get_status());
}
   ...
   run__with_a_successful_build__will_set_the_status_to_finished)
{
   BuildsBaseTestClass base_build;
   base_build.run();
   EXPECT_EQ(Blast::BuildSystem::Builds::Base::STATUS_FINISHED, base_build.get_status());
}
   ...
   run__with_a_failed_build__will_set_the_status_to_failed)
{
   ...
   base_build.run();
   EXPECT_EQ(Blast::BuildSystem::Builds::Base::STATUS_ERROR, base_build.get_status());
}
   ...
   run__with_build_stages__will_call_execute_on_each_build_stage)
{
   ...
   std::vector<Blast::BuildSystem::BuildStages::Base*> build_stages = base_build.get_build_stages();
   ...
   run__when_a_build_stage_throws_an_exception__will_halt_execution_of_the_remaining_stages)
{
   ...
   EXPECT_EQ(Blast::BuildSystem::BuildStages::Base::STATUS_SUCCEEDED, build_stages[0]->get_status());
   EXPECT_EQ(Blast::BuildSystem::BuildStages::Base::STATUS_SUCCEEDED, build_stages[1]->get_status());
   EXPECT_EQ(Blast::BuildSystem::BuildStages::Base::STATUS_ERROR, build_stages[2]->get_status());
   EXPECT_EQ(Blast::BuildSystem::BuildStages::Base::STATUS_WAITING_TO_START, build_stages[3]->get_status());
}
   ...
   run__when_a_build_stage_fails__will_halt_execution_of_the_remaining_stages)
{
   ...
   EXPECT_EQ(Blast::BuildSystem::BuildStages::Base::STATUS_SUCCEEDED, build_stages[0]->get_status());
   EXPECT_EQ(Blast::BuildSystem::BuildStages::Base::STATUS_SUCCEEDED, build_stages[1]->get_status());
   EXPECT_EQ(Blast::BuildSystem::BuildStages::Base::STATUS_FAILED, build_stages[2]->get_status());
   EXPECT_EQ(Blast::BuildSystem::BuildStages::Base::STATUS_WAITING_TO_START, build_stages[3]->get_status());
}
