...
ifeq ($(OS),Windows_NT)
  ...
  WARNINGS_PROMOTED_TO_ERRORS_FLAGS=$(ERROR_IF_UNINITIALIZED) $(ERROR_IF_INCORRECT_RETURN_TYPE) $(ERROR_IF_CONSTRUCTOR_INIT_OUT_OF_ORDER) $(ERROR_IF_ACCIDENTAL_OVERLOAD) $(ERROR_IF_VIRTUAL_NOT_MARKED_OVERRIDE) $(ERROR_IF_MACRO_REDEFINED) $(ERROR_IF_ASSIGNMENT_IN_CONDITIONAL) $(ERROR_ON_HEADER_HYGINE)
...
.deps/%.d: src/%.cpp
	...
	@g++ -std=c++17 -MM $< > $@ -I./include -I$(ASIO_INCLUDE_DIR) -I$(ALLEGRO_INCLUDE_DIR) -I$(ALLEGRO_PLATFORM_INCLUDE_DIR) $(ALLEGRO_FLARE_BUILD_ARGS) $(CURL_BUILD_ARGS) -I$(YAML_CPP_INCLUDE_DIR)
...
obj/tests/$(TEST_RUNNER_PROGRAM_NAME).o: tests/$(TEST_RUNNER_PROGRAM_NAME).cpp
	...
	@g++ -g -c -std=c++17 $(DISABLE_UNUSED_LINK_ARGUMENTS_WARNING_FLAG) $(WARNINGS_PROMOTED_TO_ERRORS_FLAGS) -Wall -Wuninitialized -Weffc++ $(DISABLE_UNUSED_WARNINGS_FLAG) $< -o $@ -I$(ALLEGRO_INCLUDE_DIR) -I$(ALLEGRO_PLATFORM_INCLUDE_DIR) -I$(GOOGLE_TEST_INCLUDE_DIR) -I$(GOOGLE_MOCK_INCLUDE_DIR) $(NCURSES_BUILD_ARGS)
...
bin/run_all_tests: $(TEST_OBJECTS) obj/tests/$(TEST_RUNNER_PROGRAM_NAME).o
	@mkdir -p $(@D)
	echo $^ > tmp/mah_obj_files.txt
	@printf "Compiling run_all_tests executable \e[1m\e[36m$@\033[0m\n"
	@g++ -g -std=c++17 $(ERROR_LIMIT_FLAG) $(DISABLE_UNUSED_LINK_ARGUMENTS_WARNING_FLAG) -Wall -Weffc++ $(WARNINGS_PROMOTED_TO_ERRORS_FLAGS) $(DISABLE_UNUSED_WARNINGS_FLAG) @tmp/mah_obj_files.txt -L./lib -l$(LIBRARY_FOR_TESTS_FOR_LINKING_NAME) $(NCURSES_BUILD_ARGS) $(NCURSES_LINK_ARGS) -I./include -I$(ASIO_INCLUDE_DIR) -I$(YAML_CPP_INCLUDE_DIR) -L$(YAML_CPP_LIB_DIR) -l$(YAML_CPP_LIBS) $(REQUIRED_WINDOWS_NETWORK_FLAGS) -L$(ALLEGRO_LIB_DIR) $(ALLEGRO_FLARE_BUILD_ARGS) $(ALLEGRO_FLARE_LINK_ARGS) $(CURL_BUILD_ARGS) $(CURL_LINK_ARGS) $(SQLITE_LINK_ARGS) -l$(GOOGLE_MOCK_LIBS) -l$(GOOGLE_TEST_LIBS) -I$(ALLEGRO_INCLUDE_DIR) -I$(ALLEGRO_PLATFORM_INCLUDE_DIR) $(ALLEGRO_LIBS_LINK_MAIN_ARGS) -D_XOPEN_SOURCE_EXTENDED $(OPENGL_LIB) $(MIDI_AUDIO_LIB) $(RPATH_STATEMENT)
