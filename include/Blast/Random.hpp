...
namespace Blast
{
   ...
   public:
      ...
      // If , then the same letter would be returned but in a lowercase.
      ...
      T get_random_element(std::vector<T> &elements)
      {
         ...
         if (elements.empty())
         {
            throw std::runtime_error("[Blast::Random::get_random_elements] error: elements cannot be empty.");
         }
      ...
      // The palette consists of the list of Blast::Color constants, which are from the X11 color codes.
