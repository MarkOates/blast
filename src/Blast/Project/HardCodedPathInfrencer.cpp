...
std::string HardCodedPathInfrencer::get_git_command()
{
   if (!(std::filesystem::exists(project_directory)))
   {
      ...
      error_message << "[Blast::Project::HardCodedPathInfrencer::get_git_command]: error: guard \"std::filesystem::exists(project_directory)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Blast::Project::HardCodedPathInfrencer::get_git_command]: error: guard \"std::filesystem::exists(project_directory)\" not met");
   }
