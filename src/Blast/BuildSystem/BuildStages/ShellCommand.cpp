...
ShellCommand::ShellCommand(std::string shell_command)
   : Blast::BuildSystem::BuildStages::Base(Blast::BuildSystem::BuildStages::ShellCommand::TYPE)
...
bool ShellCommand::execute()
{
   if (!((!executed)))
   {
      ...
      error_message << "[Blast::BuildSystem::BuildStages::ShellCommand::execute]: error: guard \"(!executed)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Blast::BuildSystem::BuildStages::ShellCommand::execute]: error: guard \"(!executed)\" not met");
   }
