...
void Base::set_build(Blast::BuildSystem::Builds::Base* build)
{
   this->build = build;
}
...
Blast::BuildSystem::Builds::Base* Base::get_build() const
{
   return build;
}
...
std::string Base::render_text()
{
   if (!(build))
   {
      ...
      error_message << "[Blast::BuildSystem::ReportRenderers::Base::render_text]: error: guard \"build\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Blast::BuildSystem::ReportRenderers::Base::render_text]: error: guard \"build\" not met");
   }
