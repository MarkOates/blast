...
std::string BuildStageRenderer::build_text_report()
{
   if (!(build_stage))
   {
      ...
      error_message << "[Blast::BuildSystem::BuildStageRenderer::build_text_report]: error: guard \"build_stage\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Blast::BuildSystem::BuildStageRenderer::build_text_report]: error: guard \"build_stage\" not met");
   }
   std::string result;
