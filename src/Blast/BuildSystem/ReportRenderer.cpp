...
std::string ReportRenderer::build_text_report()
{
   if (!(build))
   {
      ...
      error_message << "[Blast::BuildSystem::ReportRenderer::build_text_report]: error: guard \"build\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[Blast::BuildSystem::ReportRenderer::build_text_report]: error: guard \"build\" not met");
   }
   std::string result;
